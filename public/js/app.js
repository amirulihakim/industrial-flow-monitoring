(function exposeApp(root, factory) {
  ...
  function collectElements(documentObject) {
    const ids = { deviceSelect: 'device-select', pumpStatus: 'pump-status', connectionState: 'connection-state', connectionMessage: 'connection-message', statusTimestamp: 'status-timestamp', positiveTotal: 'positive-total', negativeTotal: 'negative-total', heatingTotal: 'heating-total', coolingTotal: 'cooling-total', sourceBannerTitle: 'source-banner-title', sourceBannerText: 'source-banner-text' };
  ...
  class DashboardController {
    ...
    #renderState(state, { appendChartPoint }) {
      ...
      this.elements.sourceBannerText.textContent = 'Synthetic telemetry generated by ESP32';
  ...
  return { DashboardController, STALE_AFTER_MS, STALE_CHECK_INTERVAL_MS, STOPPED_FLOW_THRESHOLD, collectElements };
}));
